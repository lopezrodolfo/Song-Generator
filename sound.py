...
class StereoSample:
    ...
    def __str__(self):
        ...
        return (
            "Sample at index "
            + str(self.__index)
            + " with a left channel value of "
            + str(self.left)
            + " and a right value value of "
            + str(self.right)
        )
...
class Sound:
    ...
    def insert(self, snd, i):
        ...
        if self.get_channels() != snd.get_channels():
            ...
            new_samples = numpy.concatenate((first_chunk, snd.__samples, second_chunk))
    ...
    def display(self, figure_title=None):
        ...
        """
        time = numpy.linspace(
            0, len(self.__samples) / self.__sample_rate, num=len(self.__samples)
        )
